...
function save(){ STORE.set('lct_items',items); STORE.set('lct_users',users); }

// seed default data
(function(){
  if(!users.length){
    users = [
      {id:'u001',fn:'Admin',ln:'User',username:'admin',email:'admin@gordon.edu.ph',password:'admin123',role:'admin',joined:new Date().toISOString().slice(0,10)},
      {id:'u002',fn:'Juan',ln:'Dela Cruz',username:'student',email:'student@gordon.edu.ph',password:'student123',role:'user',joined:new Date().toISOString().slice(0,10)}
    ];
    save();
  }
  if(!items.length){
    items=[
      {id:'LCT-1001',type:'lost',name:'Black Wallet',category:'Wallets / IDs',desc:'Leather wallet with driver license and credit cards',location:'Library - 3rd Floor',date:'2026-04-15',reporter:'John Smith',reporterId:'u002',status:'active'},
      {id:'LCT-1002',type:'found',name:'Apple AirPods',category:'Electronics',desc:'White Apple AirPods Pro in charging case',location:'Canteen',date:'2026-04-16',reporter:'Maria Garcia',reporterId:'u002',status:'active'},
      {id:'LCT-1003',type:'lost',name:'Blue Backpack',category:'Bags',desc:'Navy blue backpack with laptop compartment',location:'Entrance',date:'2026-04-14',reporter:'Alex Johnson',reporterId:'u002',status:'active'},
      {id:'LCT-1004',type:'found',name:'Physics Textbook',category:'Books / Notes',desc:'University Physics 2nd Edition - has name inside',location:'Library',date:'2026-04-16',reporter:'Prof. David Lee',reporterId:'u002',status:'active'},
      {id:'LCT-1005',type:'lost',name:'Gold Ring',category:'Jewelry',desc:'Gold engagement ring with diamond',location:'Library - Locker Area',date:'2026-04-10',reporter:'Emma Wilson',reporterId:'u002',status:'claimed'}
    ];
    save();
  }
})();
...
function handleSignup(){
  const fn=v('signupFN'),ln=v('signupLN'),un=v('signupUser'),em=v('signupEmail'),p=v('signupPass'),p2=v('signupPass2');
  ...
  users.push({id:'u'+Date.now(),fn,ln,username:un,email:em,password:p,role:'user',joined:new Date().toISOString().slice(0,10)});
  save();
  clearErr('signupErr');
...
function saveSettings(){
  ...
  if(np){if(np.length<5){toast('❌','Password must be at least 5 chars!');return;}currentUser.password=np;}
  const idx=users.findIndex(u=>u.id===currentUser.id);
  if(idx>=0) users[idx]={...users[idx],...currentUser};
  save(); updateProfileUI(); toast('✅','Settings saved!');
}
...
function refreshProfileData(){
  ...
  if(myItems.length===0){actEl.innerHTML='<div style="color:var(--muted);font-size:13px;padding:12px;">No activity yet.</div>';return;}
  ...
  document.getElementById('myReportsBody').innerHTML=myItems.length?myItems.map(i=>`
    <tr><td>${i.id}</td><td>${i.name}</td><td><span class="badge badge-${i.type}">${i.type.toUpperCase()}</span></td><td><span class="badge badge-${i.status==='claimed'?'claimed':i.type}">${i.status.toUpperCase()}</span></td></tr>`).join(''):'<tr><td colspan="4" class="empty-row">No reports yet.</td></tr>';
  // claimed table
  document.getElementById('myClaimedBody').innerHTML=myClaimed.length?myClaimed.map(i=>`
    <tr><td>${i.id}</td><td>${i.name}</td><td>${i.date}</td></tr>`).join(''):'<tr><td colspan="3" class="empty-row">No claimed items yet.</td></tr>';
  // notifications
  const notifs=myItems.filter(i=>i.status==='claimed');
  document.getElementById('notifList').innerHTML=notifs.length?notifs.map(i=>`
    <div class="activity-item"><div class="activity-dot claimed"></div><div class="activity-text">Your report <b>${i.id}</b> (${i.name}) has been marked as claimed.</div></div>`).join(''):'<div style="color:var(--muted);font-size:13px;padding:12px;">No notifications.</div>';
}
...
function openReportModal(type){
  ...
  document.getElementById('fLoc').value='';document.getElementById('fCat').value='';
...
function submitReport(){
  ...
  if(!name||!cat||!desc||!loc||!date||!rep){showErr('reportErr','Please fill in all required fields!');return;}
  const id='LCT-'+Math.floor(1000+Math.random()*9000);
  items.unshift({id,type,name,category:cat,desc,location:loc,date,reporter:rep,reporterId:currentUser?.id||'guest',status:'active'});
  save();
...
function deleteItem(id){
  ...
  save(); refreshAdmin(); toast('🗑','Item deleted.');
}
...
function deleteUser(id){
  ...
  save(); refreshAdmin(); toast('🗑','User deleted.');
}
